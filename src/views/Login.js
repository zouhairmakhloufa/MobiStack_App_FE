...
const Login = () => {
  ...
  const handleSubmit = (event) => {
    ...
    axios.post('http://localhost:5000/api/user/authenticate', formData).then((res) => {
      if (res.data.message == "2") {
        localStorage.setItem('userData', JSON.stringify(res.data.token))
        toast.success('Welcome To MobiStack 🙌');
        navigate('/')
    ...
    <div className="auth-wrapper auth-cover">
      <Row className="auth-inner m-0">
        <Link className="brand-logo" to="/" onClick={(e) => e.preventDefault()}>
          ...
          <h2 className="brand-text text-primary ms-1">MobiStack</h2>
        </Link>
